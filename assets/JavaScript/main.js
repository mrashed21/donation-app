...
donateQuota.addEventListener("click", () => {
  const amountQuota = getdonateAmount("amountQuota");
  let initialBalanceQuota = donatInitial("initialBalanceQuota");
  if (
    amountQuota === "" ||
    isNaN(amountQuota) ||
    amountQuota <= 0 ||
    amountQuota > initialBalance
  ) {
    const modal = document.getElementById("donateFailed");
    const closePopUp = document.getElementById("close-popup");
    modal.close();
    ...
    /* Update Donate Amount */
    let result = parseFloat(initialBalanceQuota) + amountQuota;
    document.getElementById("initialBalanceQuota").innerText = result;

    /*update Current Balance*/
    initialBalance -= amountQuota;
    document.getElementById("initialBalance").innerText = initialBalance;
    ...
    div.innerHTML = `<h2 class="font-medium text-xl px-4 mt-4 primary-txt">
              ${amountQuota} Donate for Flood Relief in Feni,Bangladesh
            </h2>
            <p
              class="text-base font-light secondarytxt px-4 mb-4"
            >
